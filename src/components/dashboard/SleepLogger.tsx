...
const SleepLogger = ({ todayLog, onSave }: SleepLoggerProps) => {
  ...
  const handleSave = async () => {
    setSaving(true);
    await onSave({ sleep_hours: hours, sleep_quality: quality });
    setSaving(false);
  };
      ...
      <CardContent className="space-y-6">
        ...
        <div>
          <div className="flex justify-between items-baseline mb-3">
            <p className="text-sm font-medium text-muted-foreground">Hours slept</p>
          ...
          <Slider
            ...
            className="my-2"
          />
          <p className={`text-xs font-medium mt-1 ${sleepInfo.color}`}>{sleepInfo.msg}</p>

          {/* Visual sleep bar */}
          <div className="mt-3 h-8 rounded-lg bg-muted overflow-hidden flex">
            {Array.from({ length: 14 }, (_, i) => (
              <div
                key={i}
                className={`flex-1 border-r border-background/20 transition-all ${
                  i < hours
                    ? i < 5
                      ? "bg-destructive/60"
                      : i < 7
        ...
        {/* Quality */}
        <div>
          <p className="text-sm font-medium text-muted-foreground mb-3">Sleep quality</p>
