...
const WellnessDashboard = ({ insights, logs }: WellnessDashboardProps) => {
  ...
  // Prepare chart data (last 14 days, reversed for chronological order)
  const chartData = logs.slice(0, 14).reverse().map((log) => ({
    date: new Date(log.log_date).toLocaleDateString("en", { month: "short", day: "numeric" }),
    mood: log.mood_score ? log.mood_score * 20 : null, // normalize to 0-100
    energy: log.energy_level ? log.energy_level * 10 : null,
    stress: log.stress_level ? (10 - log.stress_level) * 10 : null, // invert stress
    motivation: log.motivation_score ? log.motivation_score * 10 : null,
  }));
        ...
        <CardContent className="pt-6">
          <div className="flex flex-col sm:flex-row items-center gap-6">
            ...
            <div className="relative w-32 h-32 flex-shrink-0">
              <svg className="w-full h-full -rotate-90" viewBox="0 0 100 100">
                ...
                <circle
                  ...
                  strokeWidth="8"
                  strokeLinecap="round"
                  strokeDasharray={`${insights.wellnessScore * 2.64} 264`}
                  className="transition-all duration-1000"
                />
              </svg>
              <div className="absolute inset-0 flex flex-col items-center justify-center">
                <span className={`text-3xl font-bold ${getScoreColor(insights.wellnessScore)}`}>
            ...
            <div className="text-center sm:text-left flex-1 space-y-2">
              ...
              <p className="text-lg font-medium">{getScoreLabel(insights.wellnessScore)}</p>
              <div className="flex flex-wrap gap-4 justify-center sm:justify-start">
                ...
                <div className="flex items-center gap-1.5">
                  ...
                  <span className="text-sm font-medium">{insights.totalLogs} total logs</span>
                </div>
              </div>
            </div>
          </div>
        ...
        <div className="grid md:grid-cols-2 gap-4">
          ...
          <Card>
            <CardHeader className="pb-2">
              <CardTitle className="text-sm">14-Day Wellness Trends</CardTitle>
            </CardHeader>
            <CardContent>
              <ResponsiveContainer width="100%" height={220}>
                <AreaChart data={chartData}>
                  ...
                  <Area type="monotone" dataKey="energy" name="Energy" stroke="hsl(var(--chart-4))" fill="url(#energyGrad)" strokeWidth={2} connectNulls />
          ...
          <Card>
            <CardHeader className="pb-2">
              <CardTitle className="text-sm">Wellness Radar</CardTitle>
            </CardHeader>
            <CardContent>
              <ResponsiveContainer width="100%" height={220}>
                ...
                <div key={log.id} className="flex items-center justify-between p-2.5 rounded-lg bg-muted/50 text-sm">
                  <div className="flex items-center gap-3">
                    <span className="text-lg">
                      {log.mood_score === 5 ? "🤩" : log.mood_score === 4 ? "😊" : log.mood_score === 3 ? "😐" : log.mood_score === 2 ? "😔" : log.mood_score === 1 ? "😞" : "—"}
                    </span>
                    <div>
                      ...
                      {log.tags.length > 0 && (
                        <p className="text-xs text-muted-foreground">{log.tags.slice(0, 3).join(", ")}</p>
                      )}
                    </div>
                  </div>
