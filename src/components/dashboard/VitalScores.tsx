...
const VitalScores = ({ todayLog, onSave }: VitalScoresProps) => {
  ...
  const getColor = (val: number, inverse = false) => {
    const v = inverse ? 11 - val : val;
    if (v <= 3) return "text-destructive";
    if (v <= 5) return "text-warning";
    if (v <= 7) return "text-muted-foreground";
    return "text-success";
  };

  const getBarColor = (val: number, inverse = false) => {
    const v = inverse ? 11 - val : val;
    if (v <= 3) return "bg-destructive";
    if (v <= 5) return "bg-warning";
    if (v <= 7) return "bg-primary";
    return "bg-success";
  };
  ...
  const metrics = [
    {
      ...
      color: getColor(motivation),
      barColor: getBarColor(motivation),
      low: "Low",
      high: "On Fire",
    },
    {
    ...
    },
    {
      ...
      value: stress,
      setValue: setStress,
      color: getColor(stress, true),
      barColor: getBarColor(stress, true),
      low: "Zen",
      high: "Overwhelmed",
    },
  ];
        ...
        {metrics.map((m) => (
          <div key={m.label} className="space-y-2">
            <div className="flex items-center justify-between">
              ...
              <span className={`text-2xl font-bold ${m.color}`}>{m.value}/10</span>
            </div>
            <Slider
              value={[m.value]}
              ...
              {Array.from({ length: 10 }, (_, i) => (
                <div
                  ...
                  className={`flex-1 rounded-full transition-all ${
                    i < m.value ? m.barColor + " opacity-80" : "bg-muted"
                  }`}
                />
              ))}
            </div>
