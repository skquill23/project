    ...
    return (
      <div className="flex items-center justify-center py-16">
        <div className="flex flex-col items-center gap-3">
          ...
          <p className="text-muted-foreground text-sm">Loading wellness data...</p>
        </div>
      </div>
    );
  }
    ...
    <Tabs defaultValue="overview" className="w-full">
      <TabsList className="grid w-full grid-cols-5 mb-4 h-auto p-1">
        {[
          ...
          <TabsTrigger key={tab.value} value={tab.value} className="flex items-center gap-1.5 py-2 text-xs sm:text-sm">
      ...
      <TabsContent value="overview">
        <WellnessDashboard insights={insights} logs={logs} />
      </TabsContent>

      <TabsContent value="mood">
        <MoodTracker todayLog={todayLog} onSave={saveLog} />
      </TabsContent>

      <TabsContent value="sleep">
        <SleepLogger todayLog={todayLog} onSave={saveLog} />
      </TabsContent>

      <TabsContent value="vitals">
        <VitalScores todayLog={todayLog} onSave={saveLog} />
      </TabsContent>
